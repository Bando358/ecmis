...
export default function GestionStockProduitPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (session?.user?.role !== "ADMIN") {
          const tabCliniquesUser = session?.user?.cliniques || [];
    ...
    fetchData();
  }, [session?.user?.role, session?.user?.cliniques, status]); // Dépendances plus spécifiques
    ...
    const fetchPermissions = async () => {
      try {
        ...
        const perm = permissions.find(
          (p) => p.table === TableName.STOCK_PRODUIT
        );
