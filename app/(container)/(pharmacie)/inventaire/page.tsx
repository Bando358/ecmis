...
export default function DetailInventairePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (oneUser?.role !== "ADMIN") {
          ...
          filteredInventaires = inventairesData.filter((inventaire) =>
    ...
    fetchData();
  }, [session?.user?.role, session?.user?.cliniques, status]);
  ...
  if (status === "loading") {
    return (
      <div className="space-y-4 max-w-[1200px] p-4 flex flex-col mx-auto">
