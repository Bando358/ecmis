...
const TabContentItem = ({ fiche, fichesId }: TabContentItemProps) => {
...
const CategoryTabs = ({ category, fichesId }: CategoryTabsProps) => (
  <Tabs defaultValue={category.defaultTab} className="w-full mt-6">
    ...
    {category.fiches.map((fiche, index) => (
      <TabContentItem key={index} fiche={fiche} fichesId={fichesId} />
    ))}
  </Tabs>
);
...
export default function FichesClient({
  ...
  const incrementDate = useCallback(() => {
    setCurrentIndex((prev) => (prev < visites.length - 1 ? prev + 1 : prev));
  }, [visites.length]);
  ...
  const currentRecap = useMemo(() => {
    const currentVisite = visites[currentIndex];
    if (!currentVisite) return undefined;
    return recaps.find((r) => r.idVisite === currentVisite.id);
  }, [recaps, currentIndex, visites]);
          ...
          <CardHeader className="pb-3 bg-blue-50/40">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                >
                  {visites.length.toString().padStart(2, "0")}
                </Badge>
              </div>
