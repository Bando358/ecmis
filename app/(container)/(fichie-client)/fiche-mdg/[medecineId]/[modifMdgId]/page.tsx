  ...
  useEffect(() => {
    ...
    const fetchPermissions = async () => {
      try {
        ...
        const perm = permissions.find((p) => p.table === TableName.MEDECINE);
    ...
    const fetchData = async () => {
      try {
        ...
        if (oneMedecine) {
          ...
          setVisites(result.filter((r) => r.id === oneMedecine.mdgIdVisite));

          // 4. Trouver la visite spécifique associée à cette médecine
          const visiteDate = result.find(
            (r) => r.id === oneMedecine.mdgIdVisite
          );
              ...
              <div>
                {dateVisite ? (
                  ...
                  <div className="space-x-2 flex flex-row">
                    ...
                    <Skeleton className="h-4 w-[100px]  bg-slate-600" />
                  </div>
                )}
              </div>
