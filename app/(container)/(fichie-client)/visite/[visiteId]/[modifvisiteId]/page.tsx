...
export default function FormVisiteModification({
    ...
    const fetchData = async () => {
      try {
        ...
        if (oneVisiteData?.idClient) {
          ...
          const idActiviteArray = allActivite.map((a) => a.id);
          setLoadingLieu(true);
  ...
  useEffect(() => {
    ...
    const fetchPermissions = async () => {
      try {
        ...
        const perm = permissions.find((p) => p.table === TableName.VISITE);
  ...
  return (
    <div className="flex flex-col w-full justify-center max-w-[900px] mx-auto px-4 py-2 rounded-md">
      {isVisible ? (
        <>
          ...
          <Form {...form}>
            <form
              ...
              className="space-y-4 max-w-[900px] rounded-sm mx-auto px-4 py-2 bg-white shadow-md"
            >
              <FormField
