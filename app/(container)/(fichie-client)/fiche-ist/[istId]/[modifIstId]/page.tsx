  ...
  useEffect(() => {
    const fetchData = async () => {
      if (selectedIst) {
        ...
        const visiteDate = result.find((r) => r.id === selectedIst.istIdVisite);

        const nomPrescripteur = await getOneUser(selectedIst.istIdUser);
        setPrescripteur(nomPrescripteur?.name);

        setVisites(result.filter((r) => r.id === selectedIst.istIdVisite)); // Assurez-vous que result est bien de type CliniqueData[]
