...
export default function OrdonnancePage({
  ...
  useEffect(() => {
    const prestataire = session?.user.prestataire;
    if (prestataire != undefined) {
      setIsPrestataire(prestataire);
    }
  }, [session]);
  ...
  useEffect(() => {
    ...
    const fetchPermissions = async () => {
      try {
        ...
        const perm = permissions.find((p) => p.table === TableName.ORDONNANCE);
  ...
  const form = useForm<Ordonnance>({
    defaultValues: {
      ...
      ordonnanceIdUser: isPrestataire === true ? idUser || "" : "",
    },
  });
  ...
  const getOrdonnanceByIdVisite = useCallback(
    ...
    [tabOrdonnance, form, isPrestataire, idUser, ordonnanceId]
  );
  ...
  const handleUpdate = () => {
    if (!isPermissionUpdated && session?.user.role !== "ADMIN") {
      alert(
