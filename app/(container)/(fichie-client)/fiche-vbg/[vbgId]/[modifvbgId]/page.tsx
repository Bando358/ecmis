  ...
  useEffect(() => {
    ...
    const fetchPermissions = async () => {
      try {
        ...
        const perm = permissions.find((p) => p.table === TableName.VBG);
    ...
    const fetchData = async () => {
      if (selectedOneVbg) {
        ...
        const visiteDate = result.find(
          (r) => r.id === selectedOneVbg.vbgIdVisite
        );

        const nomPrescripteur = await getOneUser(selectedOneVbg.vbgIdUser);
        setPrescripteur(nomPrescripteur?.name);

        setVisites(result.filter((r) => r.id === selectedOneVbg.vbgIdVisite)); // Assurez-vous que result est bien de type CliniqueData[]
