...
export default function PageDemandeExamen({
  ...
  useEffect(() => {
    if (session?.user && client) {
      const fetchPrescripteurs = async () => {
        ...
        const prestataire = await getOneUser(session.user.id as string);
        setPrescripteurs(prestataire);
        if (prestataire && !prestataire.prescripteur) {
          setTabPrescripteurs([prestataire, ...prescripteursData]);
        }
      };
    ...
    const fetchPermissions = async () => {
      try {
        ...
        const perm = permissions.find(
          (p) => p.table === TableName.DEMANDE_EXAMEN
        );
  ...
  const handleDemandeExamen = async () => {
    ...
    try {
      ...
      setDemandes(newDemandes.filter((d) => d.idVisite === selectedVisite));
      ...
      <div className="mb-6">
        <div className="flex items-center gap-4 mb-4">
          <div className="flex flex-justify-start items-center gap-2">
            ...
            {selectedVisite && !session?.user.prescripteur && (
              <select
                value={selectedPrescripteur}
                onChange={(e) => setSelectedPrescripteur(e.target.value)}
                className="border rounded-md px-4 py-2 max-w-[200px]"
              >
