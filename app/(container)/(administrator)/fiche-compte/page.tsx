...
import { Clinique, TableName, User } from "@/lib/generated/prisma";
  ...
  useEffect(() => {
    ...
    const fetchPermissions = async () => {
      try {
        ...
        const perm = permissions.find((p) => p.table === TableName.USER);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (userRole === "ADMIN") {
        ...
        const alluser = result.filter((user) => user.role === "USER");
        const userTableId = result.find((user) => user.id === idUser); // Utilisez result au lieu de allUser
    ...
    const fetchData = async () => {
      try {
        ...
        if (userRole === "ADMIN") {
          ...
          if (oneUser) {
            const filteredCliniques = clinique.filter((c) =>
