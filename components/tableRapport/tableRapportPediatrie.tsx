...
export default function TableRapportPediatrie({
  ...
  const exportToPdf = async () => {
    ...
    try {
      ...
      serviceRows.push(["SRV - Total service pédiatrie", ...srvTotalMasc.map(String), ...srvTotalFem.map(String), String(srvGrandTotal)]);
      ...
      <Table className="border">
        ...
        <TableBody>
          ...
          <TableRow className="font-bold bg-slate-200">
            <TableCell>SRV - Total service pédiatrie</TableCell>
            {ageRanges.map((range, index) => (
