...
export type convertedType = clientDataProps & {
  depistageVihTypeClientCdip: boolean;
  depistageVihResultatPositif: boolean;
  depistageVihPtme: boolean;
  depistageVihPtmePositif: boolean;
  depistageVihPtmePositifMisSousArv: boolean;
  depistageVihPtmeEnfants: boolean;
  depistageVihPtmeEnfantsPositifMisSousArv: boolean;
};
...
const tabClientDepistage = [
  {
    label: "Nombre de personnes reçues",
    value: "depistageVihConsultation",
  },
  {
    label: "Nombre de personnes reçues pour le CDV/CDIP",
    value: "depistageVihTypeClientCdip",
  },
  {
    label: "Nombre de personnes dépistées VIH+",
    value: "depistageVihResultatPositif",
  },
  {
    label: "Nombre de femmes enceintes dépistées (PTME)",
    value: "depistageVihPtme",
  },
  {
    label: "Nombre de femmes enceintes dépistées VIH+ (PTME)",
    value: "depistageVihPtmePositif",
  },
  {
    label: "Nombre de femmes enceintes dépistées VIH+ mises sous ARV (PTME)",
    value: "depistageVihPtmePositifMisSousArv",
  },
  {
    label: "Nombre d'enfants de mères VIH+ dépistés (PTME)",
    value: "depistageVihPtmeEnfants",
  },
  {
    label: "Nombre d'enfants de mères VIH+ dépistés VIH+ mises sous ARV (PTME)",
    value: "depistageVihPtmeEnfantsPositifMisSousArv",
  },
];
const tabServiceDepistage = [
  { label: "SRV - VIH - Consultation", value: "depistageVihConsultation" },
  {
    label: "SRV - VIH - Counseling avant dépistage",
    value: "depistageVihCounsellingPreTest",
  },
  {
    label: "SRV - VIH - Investigation - Analyse Test Rapide de diagnostic",
    value: "depistageVihInvestigationTestRapide",
  },
  {
    label: "SRV - VIH - Counseling après dépistage",
    value: "depistageVihCounsellingPostTest",
  },
  {
    label: "SRV - VIH - Counseling Réduction des Risques",
    value: "depistageVihCounsellingReductionRisque",
  },
  {
    label: "SRV - VIH - Counseling Soutien Psycho-Social",
    value: "depistageVihCounsellingSoutienPsychoSocial",
  },
];
export default function TableRapportDepistageVih({
  ageRanges,
  clientData,
  dateDebut,
  dateFin,
  ...
  useEffect(() => {
    if (!clientData || clientData.length === 0) {
      ...
      const newConverted = clientData.map((item) => ({
        ...
        depistageVihTypeClientCdip: Boolean(
          item.depistageVihTypeClient && item.depistageVihTypeClient !== "ptme"
        ), // ⚡
        depistageVihResultatPositif:
          item.depistageVihResultat === "positif" &&
          item.depistageVihTypeClient !== "ptme", // ⚡
        depistageVihPtme: item.depistageVihTypeClient === "ptme", // ⚡
        depistageVihPtmePositif:
          item.depistageVihTypeClient === "ptme" &&
          item.depistageVihResultat === "positif", // ⚡
        depistageVihPtmePositifMisSousArv:
          item.depistageVihTypeClient === "ptme" &&
          item.depistageVihResultat === "positif" &&
          item.depistageVihResultatPositifMisSousArv === true, // ⚡
        depistageVihPtmeEnfants:
          item.depistageVihTypeClient === "enfantMerePos",
        depistageVihPtmeEnfantsPositifMisSousArv:
          item.depistageVihTypeClient === "enfantMerePos" &&
          item.depistageVihResultatPositifMisSousArv === true, // ⚡
      }));
  ...
  const exportToExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("Depistage VIH");
    ...
    tableExportWithSexe(
      "Rapport clients Reçus Depistage VIH",
      worksheet,
    ...
    // Tableau des services VIH -
    tableExportWithSexe(
      "Rapport services Depistage VIH",
      worksheet,
    ...
    link.download = `Rapport_DEPISTAGE_VIH_${new Date().toLocaleDateString(
      "fr-FR"
    )}.xlsx`;
    link.click();
  ...
  const exportToPdf = async () => {
    ...
    // Tableau 1: Rapport clients Dépistage VIH
    doc.setFontSize(11);
    doc.setFont("helvetica", "bold");
    doc.text("Rapport clients Dépistage VIH", 14, currentY);
    currentY += 5;

    const bodyClientsDepistage = tabClientDepistage.map((item) => {
    ...
    // Tableau 2: Rapport services Dépistage VIH
    doc.setFontSize(11);
    doc.setFont("helvetica", "bold");
    doc.text("Rapport services Dépistage VIH", 14, currentY);
    currentY += 5;

    const bodyServicesDepistage = tabServiceDepistage.map((item) => {
      const masculinValues = ageRanges.map((range) =>
        countClientBooleanBySexe(converted, range.min, range.max, item.value, true, "Masculin")
      );
      const femininValues = ageRanges.map((range) =>
    ...
    doc.save(`Rapport_DEPISTAGE_VIH_${new Date().toLocaleDateString("fr-FR")}.pdf`);
    setSpinnerPdf(false);
  };
  ...
  return (
    <div className="flex flex-col gap-4 bg-gray-50 opacity-90 p-4 rounded-sm mt-2 w-full overflow-x-auto">
      ...
      <h2 className="font-bold">Rapport clients VIH -</h2>
      ...
      <h2 className="font-bold">Rapport services VIH -</h2>
      <Table className="border">
        <TableHeader className="bg-gray-200">
          <TableRow>
            <TableCell rowSpan={2} className="font-bold">
        ...
        <TableBody>
          {tabServiceDepistage.map((client) => (
            <TableRow key={client.label}>
              {/* Nom du produit */}
              <TableCell>{client.label}</TableCell>
