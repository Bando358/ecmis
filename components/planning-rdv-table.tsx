// components/planning-rdv-table.tsx
"use client";
...
interface PlanningRdvTableProps {
...
export function PlanningRdvTable({
  cliniques,
  clinicIds,
  activites,
  dateDebut,
  dateFin,
}: PlanningRdvTableProps) {
  const [data, setData] = useState<ClientDataPlanning[]>([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const clients = await fetchClientsPlanningRDV(
          cliniques,
          clinicIds,
          activiteIds,
          dateDebut,
        ...
        setAllGestionVisite(
          gestionVisites.filter(
            (gv: { typeVisite: string }) =>
              gv.typeVisite === TypeVisite.CONTRACEPTION
          ) as GestionVisite[]
        );
      } catch (error) {
  ...
  const columns: ColumnDef<ClientDataPlanning>[] = [
    {
      accessorKey: "nom",
      header: "Nom",
      cell: ({ row }) => (
    ...
    {
      accessorKey: "methodePrise",
      header: "Méthode Prise",
    },
    {
      accessorKey: "consultationPf",
      header: "Consultation PF",
      cell: ({ row }) => (row.getValue("consultationPf") ? "Oui" : "Non"),
    },
    {
      accessorKey: "counsellingPf",
      header: "Counselling PF",
      cell: ({ row }) => (row.getValue("counsellingPf") ? "Oui" : "Non"),
    },
    {
      accessorKey: "rdvPf",
      header: "RDV PF",
      cell: ({ row }) => new Date(row.getValue("rdvPf")).toLocaleDateString(),
    },
    {
      accessorKey: "visiteSup",
      header: "Visite Sup",
      cell: ({ row }) => {
        const dateValueRdv = row.getValue("rdvPf");
                      ...
                      <TooltipTrigger asChild>
                        <div className="inline-block">
                          <DialogGestionVisite
                            ...
                            typeVisite={TypeVisite.CONTRACEPTION}
