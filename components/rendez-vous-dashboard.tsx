...
import { Lieu } from "@/lib/generated/prisma";
...
export default function RendezVousDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const cliniqueOptions = allCliniques.map((clinique) => ({
        value: clinique.id,
  ...
  useEffect(() => {
    const fetchActivites = async () => {
      ...
      try {
        ...
        const activiteIds = activitesData.map((activite) => activite.id);
  ...
  return (
    <div className="flex flex-col p-6 w-full">
            ...
            >
              ...
              <div className="flex justify-center mt-6">
                <button
                  type="submit"
                  disabled={loading}
                  className="px-6 py-2 bg-blue-500 text-white rounded-md hover:bg-blue-600 disabled:bg-blue-300 flex items-center gap-2 min-w-[120px] justify-center"
                >
                  <Spinner
                    show={loading}
                    size={"small"}
      ...
      {filters.clinicIds.length > 0 ? (
        <Card className="max-w-[960px] min-w-[960px] mx-auto bg-transparent border-none">
          <CardHeader>
