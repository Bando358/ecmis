...
export default function CommissionExamenTotalRapport({
  ...
  const filteredRows = useMemo(() => {
    ...
    if (!q) return commissionsExamenTotal;
    return commissionsExamenTotal.filter(
      (d) =>
        d.prescripteur.toLowerCase().includes(q) ||
        (d.contact || "").toLowerCase().includes(q),
    );
  }, [commissionsExamenTotal, search]);
        ...
        <>
          <div className="flex items-center gap-2 print:hidden">
            ...
            <span className="text-xs text-muted-foreground">
              {filteredRows.length} / {commissionsExamenTotal.length} prescripteur(s)
            </span>
          </div>
